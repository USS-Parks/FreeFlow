...
- Hosted evidence: run `29698366361` passed native Windows and macOS foundation
  jobs, provenance, and security at exact candidate commit
  `f1d66200f1b345a8a82157bd78965d84044dac2c`.
- Result: FF-P4 is complete and FF-P5 is next.
- Commit SHA: `f1d66200f1b345a8a82157bd78965d84044dac2c`.
...
- Hosted evidence: run `29700140823` passed native Windows and macOS
  foundation jobs, provenance, and security at exact candidate commit
  `44c9ac6b8371440e20aa440542396022dbbeb3e0`.
- Result: FF-P5 and Milestone P implementation are complete; FF-A1 is next.
- Commit SHA: `44c9ac6b8371440e20aa440542396022dbbeb3e0`.
...
- Objective: add an optional local transform provider with immutable licensed
  ...
  bounded execution, timeout, and raw-transcript fallback.
- Starting commit: `157c5e3b4482d61b0a6074365056421e3c6ab54f` on canonical
  `main`.
- Reuse classification: extension at the existing manifest downloader, Rust
  application service, post-processing, typed settings/bindings, and i18next
  seams; new pinned llama.cpp process adapter and headless/live gate.
- Files changed: local runtime service and CLI verifier; immutable model
  manifest; local-only settings migration; post-processing integration;
  consent/status/settings UI; generated bindings; 23 catalogs; workflow,
  notices, dependency ledger, and gate evidence.
- Verification commands: frozen Bun install; ESLint/service boundary; Prettier
  and Rustfmt; 552-key translation consistency; TypeScript/Vite production
  build; full Rust tests; warnings-denied Clippy; optimized Tauri no-bundle
  build; RustSec; provenance; diff checks; explicit local runtime/model install;
  CPU/Vulkan transforms; timeout and corrupt-model probes.
- Live-system evidence: combined manifest digest
...
- Objective: implement explicit cleanup strengths and original per-application
  ...
  transforms, semantic fail-closed validation, and reversible raw/final review.
- Starting commit: `595eba50be8d2ec961a7f35a65b863fcdd3f49e3` on canonical
  `main`.
- Reuse classification: extension at the FF-P4 deterministic voice-control,
  FF-P5 captured application profile, FF-A1 local runtime, history raw/final,
  typed settings/bindings, and i18next seams; original cleanup/style policy and
  semantic validator.
- Files changed: cleanup policy and fixtures; captured-target transform
  integration; fail-closed settings migration; generated bindings; cleanup and
  per-app style UI; raw/final history review and reversible cleanup; 23 locale
  catalogs; behavior matrix and gate evidence.
- Verification commands: frozen Bun install; ESLint/service boundary; Prettier
  and Rustfmt; 569-key translation consistency; TypeScript/Vite production
  build; full Rust tests; warnings-denied Clippy; optimized Tauri no-bundle
  build; RustSec; provenance; diff checks; typed-binding regeneration.
- Automated evidence: 217/217 runnable Rust tests pass with the two explicit
  731 MB live-model tests ignored. None/Light/Medium/High, names, numbers, code,
  hallucination, brevity, prompt isolation, backtrack, every style, migration,
...
- Hosted evidence: GitHub Actions run `29708191017` passed Windows and macOS
  native tests, provenance, and security at the exact candidate commit.
- Result: FF-A3 is complete and FF-A4 is next.
- Commit SHA: `e22b9022578a9cca02e37e6fb759ff8b37a58dea`.
...
- Objective: implement press-and-hold spoken local commands over a safe
  ...
  delivery, lossless copy fallback, and separate explicit confirmation before
  any supported preference change.
- Starting commit: `3cd1d3c51f49c63fe866f7991a6be0362020ae54` on canonical
  `main`.
- Reuse classification: extension at the FF-V2 coordinator/audio/cancellation,
  FF-P5 target policy, FF-A1 local runtime, FF-A3 explicit-selection,
  shortcut/settings/bindings, clipboard, and non-focus-stealing overlay seams;
  original command classifier, prompt boundary, confirmation state, and UI.
- Files changed: command-mode manager and fixtures; coordinator-owned
  hold-to-speak action; settings-schema-v7 shortcut; command request limits;
  selection/cursor delivery and copy fallback; explicit preference-confirmation
  commands; generated bindings; overlay UI; 23 locale catalogs; behavior matrix
  and gate evidence.
- Verification commands: frozen Bun install; ESLint/service boundary; Prettier
  and Rustfmt; 628-key translation consistency; TypeScript/Vite production
  build; full Rust tests; warnings-denied Clippy; optimized Tauri no-bundle
  build; RustSec; provenance; diff checks; typed-binding regeneration.
- Automated evidence: 231/231 runnable Rust tests pass with the two explicit
  731 MB live-model tests ignored. Intent isolation, preference confirmation,
  unsupported/remote rejection, prompt boundaries, shared coordinator,
  cancellation, and processing-exclusion fixtures pass. All catalogs are
  complete.
